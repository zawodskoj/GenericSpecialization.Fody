...
namespace GenericSpecialization.Tests
{
    ...
    {
        public static IEnumerable<object[]> GenericSpecializationTestInstances, LambdaTestInstances;
        ...
        static GenericSpecializationTests()
        {
            ...
            IEnumerable<object[]> CreateTestObjects(string testName)
            {
                var specType = testResult.Assembly.GetType("GenericSpecialization.AssemblyToProcess." + testName + "_Specialized");
                var specializedInstance = Activator.CreateInstance(specType);
            
                var notSpecType = testResult.Assembly.GetType("GenericSpecialization.AssemblyToProcess." + testName + "_NotSpecialized");
                var notSpecializedInstance = Activator.CreateInstance(notSpecType);

                return new[] {new[]{specializedInstance}, new[] {notSpecializedInstance}};
            }

            GenericSpecializationTestInstances = CreateTestObjects("GenericSpecializationTest");
            LambdaTestInstances = CreateTestObjects("LambdaTest");
        }
        
        [Theory, MemberData(nameof(GenericSpecializationTestInstances))]
